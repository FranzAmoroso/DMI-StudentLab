...
class _TeacherAreaPageState extends State<TeacherAreaPage> {
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      final List<Map<String, dynamic>> subjects = await _apiService
          .getTeacherSubjects();
  ...
  Future<void> _openSubject(Map<String, dynamic> subject) async {
    ...
    final String university = _firstNonEmpty([subject['university']]);
    ...
    if (subjectId == null ||
        ...
        department.isEmpty ||
        course.isEmpty) {
      _showMessage('I dati della materia non sono completi.');
      return;
    }
  ...
  Future<void> _openMaterials() async {
    ...
    await Navigator.of(context).push(
      MaterialPageRoute<void>(builder: (_) => const TeacherMaterialsPage()),
    );
  ...
  Future<void> _openNewsEditor() async {
    ...
    final bool? created = await Navigator.of(context).push<bool>(
      MaterialPageRoute<bool>(
        builder: (_) => PublicNewsEditorPage.teacher(subjects: _subjects),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    if (!_authorized) {
      return _TeacherAccessDeniedPage(error: _error, onRetry: _initialize);
    }
            ...
            child: RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  TeacherMaterialsSection(subjects: _subjects),
                  const SizedBox(height: 30),
                ],
              ),
            ),
        ...
        return GridView.count(
          ...
          children: [
            ...
            _TeacherActionCard(
              ...
              description:
                  'Visualizza le richieste di materiale ricevute per le tue materie verificate.',
              onTap: () async {
                await Navigator.of(context).push(
                  MaterialPageRoute<void>(
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: Icon(icon, color: AppColors.teacherIndigo, size: 23),
                  ),
                  const Spacer(),
                  if (pending)
                    const Text(
...
class _TeacherSubjectCard extends StatelessWidget {
  ...
  const _TeacherSubjectCard({required this.subject, required this.onTap});

  @override
  Widget build(BuildContext context) {
    final String code = subject['code']?.toString().trim() ?? '';
    final String name = subject['name']?.toString().trim() ?? 'Materia';
    final String department = subject['department']?.toString().trim() ?? '';
...
class _TeacherAccessDeniedPage extends StatelessWidget {
  ...
  const _TeacherAccessDeniedPage({required this.error, required this.onRetry});
