...
class _TeacherMaterialsPageState extends State<TeacherMaterialsPage> {
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      final List<dynamic> result = await Future.wait([
        ...
        _apiService.getTeacherMaterialRequests(),
      ]);
      ...
      setState(() {
        ...
        _materialRequests = List<Map<String, dynamic>>.from(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (_pendingMaterialRequests.isNotEmpty) ...[
                          const SizedBox(height: 18),
                          _buildMaterialRequestsSection(),
                        ],
  ...
  List<Map<String, dynamic>> get _pendingMaterialRequests => _materialRequests
      .where(
  ...
  Widget _buildMaterialRequestsSection() {
    final List<Map<String, dynamic>> requests = _pendingMaterialRequests;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            'Se hai già pubblicato il file richiesto, collega quel materiale alla richiesta. Lo studente riceverà una notifica e potrà trovarlo tra i materiali della materia.',
            style: TextStyle(
              color: Colors.white54,
              fontSize: 10.5,
              height: 1.4,
          ...
          for (final Map<String, dynamic> request in requests) ...[
            _buildMaterialRequestCard(request),
  ...
  Widget _buildMaterialRequestCard(Map<String, dynamic> request) {
    final int? requestId = _toInt(request['id']);
    final int? subjectId = _toInt(request['subject_id']);
    final String topic = request['topic']?.toString().trim() ?? '';
    final String message = request['message']?.toString().trim() ?? '';
    final int? studentId = _toInt(request['student_user_id']);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            topic.isEmpty ? 'Richiesta materiale' : topic,
            style: const TextStyle(
              color: AppColors.pureWhite,
              fontWeight: FontWeight.w700,
              fontSize: 13,
          ...
          const SizedBox(height: 8),
          Text(
            'Studente ${studentId == null ? '—' : '#$studentId'} · Materia ${subjectId == null ? '—' : '#$subjectId'}',
          ...
          Wrap(
            ...
            children: [
              FilledButton.icon(
                onPressed: _busy || requestId == null || subjectId == null
                    ? null
                    : () => _fulfillMaterialRequest(
                        requestId: requestId,
                        subjectId: subjectId,
                      ),
              ...
              OutlinedButton.icon(
                onPressed: _busy || requestId == null
                    ? null
                    : () => _rejectMaterialRequest(requestId),
                icon: const Icon(Icons.close_rounded, size: 17),
                label: const Text('Chiudi richiesta'),
              ),
            ],
          ),
        ],
  ...
  Future<void> _fulfillMaterialRequest({
    required int requestId,
    required int subjectId,
  }) async {
    final List<Map<String, dynamic>> candidates = _materials
        .where(
          (Map<String, dynamic> material) =>
              _toInt(material['subject_id']) == subjectId &&
              (material['status']?.toString().toLowerCase() ?? 'active') ==
                  'active' &&
    ...
    if (candidates.isEmpty) {
      _showMessage(
        'Non hai ancora un materiale attivo per questa materia. Caricalo prima, poi collega la richiesta.',
      );
      return;
    }

    final int? selectedMaterialId = await showModalBottomSheet<int>(
      context: context,
      backgroundColor: AppColors.eleganceDeepNavy,
      showDragHandle: true,
      builder: (BuildContext sheetContext) => SafeArea(
        child: ListView.separated(
          ...
          itemBuilder: (BuildContext context, int index) {
            final Map<String, dynamic> material = candidates[index];
            final int? materialId = _toInt(material['id']);
            final String title =
                material['title']?.toString().trim().isNotEmpty == true
            ...
            return ListTile(
              ...
              subtitle: Text(
                material['original_name']?.toString() ?? '',
                style: const TextStyle(color: Colors.white38, fontSize: 10),
              ),
              onTap: materialId == null
                  ? null
                  : () => Navigator.pop(sheetContext, materialId),
            );
          },
        ),
      ),
    ...
    setState(() => _busy = true);
    try {
      await _apiService.resolveTeacherMaterialRequest(
        requestId: requestId,
        action: 'fulfilled',
        fulfilledMaterialId: selectedMaterialId,
      );
      await _refreshAfterRequestAction();
      _showMessage('Richiesta collegata al materiale e studente avvisato.');
    } catch (error) {
  ...
  Future<void> _rejectMaterialRequest(int requestId) async {
    final bool? confirmed = await showDialog<bool>(
      context: context,
      builder: (BuildContext dialogContext) => AlertDialog(
        ...
        content: const Text(
          'La richiesta verrà chiusa senza collegare un materiale. Lo studente riceverà una notifica.',
    ...
    try {
      ...
      await _refreshAfterRequestAction();
      _showMessage('Richiesta chiusa e studente avvisato.');
    } catch (error) {
      _showMessage(_friendlyError(error));
    } finally {
      if (mounted) setState(() => _busy = false);
  ...
  Future<void> _refreshAfterRequestAction() async {
    final List<dynamic> result = await Future.wait([
      _apiService.getTeacherMaterials(),
      _apiService.getTeacherMaterialRequests(),
    ]);
